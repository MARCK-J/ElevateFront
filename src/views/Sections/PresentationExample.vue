...
const generateContent = async () => {
  ...
  // Agrupar los cursos por categoryCourseId y filtrar solo los disponibles
  ...
  data.value = categories.map((category) => {
    ...
    return {
      ...
      items: items.slice(-3), // Obtener los últimos 3 cursos de cada categoría
      hasCourses: items.length > 0, // Verificar si hay cursos disponibles
    };
  });
};
...
<template>
  <section class="my-5">
      ...
      <div class="row justify-content-center text-center mb-5">
        <div class="col-lg-8">
          ...
          <h2 class="text-primary mb-3">
            Aquí empieza tu preparación para el futuro
          </h2>
          <p class="lead text-muted">
            Estamos comprometidos con tu educación, ofreciéndote los mejores
            cursos en un solo lugar.
          </p>
        </div>
      </div>
    </div>
    <div class="container">
      <div
        v-for="({ heading, items, hasCourses }, index) in data"
        :key="heading"
          ...
          <div :class="`row ${index > 0 ? 'mt-4' : ''}`">
            <div v-if="hasCourses">
              ...
              >
                ...
                />
                <p :class="available ? 'text-success' : 'text-muted'">
                  {{ available ? "Disponible" : "No Disponible" }}
                </p>
              </div>
            </div>
            <div v-else>
              <p>Próximamente estarán disponibles los cursos.</p>
            </div>
          </div>
        </div>
      </div>
